...
- **Light theme and responsive layout (complete):** the web UI follows `prefers-color-scheme` on first load and remembers an explicit toggle after that. The light palette is warm paper rather than cold white, and its contrast was measured pair by pair against the dark theme's own ratios instead of judged by eye, which also found and fixed four contrast defects in the shipped dark theme. Below 1024px the fixed-viewport shell becomes an ordinary scrolling page and the left rail becomes a disclosure panel; wide content such as CEF samples and the reference tables scrolls inside its own container rather than pushing the page sideways.
