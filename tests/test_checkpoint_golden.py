...
"""Byte-for-byte golden tests against docs/checkpoint-cef-reference.md.

The reference document is the oracle. This builds the same eight events used by the
FortiGate and Palo Alto golden tests and asserts the CheckPointProfile + CEF serializer
reproduce each Check Point CEF payload from Section 3 exactly.
"""
...
REFERENCE = Path(__file__).resolve().parents[1] / "docs" / "checkpoint-cef-reference.md"

_GOLDEN_LINE = re.compile(r"^<\d+>.* (CEF:0\|Check Point\|.*)$")
